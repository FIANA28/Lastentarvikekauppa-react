...
orderRouter.post('/', isAuth, expressAsyncHandler(async (req, res) => {
    if(req.body.orderItems.length === 0) {
        ...
        const order = new Order({
            ...
            paymentMethod: req.body.itemsPrice,
...
orderRouter.delete('/:id', isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
    ...
    if (order) {
        ...
        res.send({ message: 'Tilaus poistettu', order: deleteOrder });
        } else {
            res.status(404).send({ message: 'Order Not Found' });
        }
    })
...
orderRouter.put('/:id/pay', isAuth, expressAsyncHandler(async (req, res) => {
    ...
    res.send({ message: 'Tilaus maksettu', order: updatedOrder });
    } else {
        res.status(404).send({ message: 'Order Not Found'});
    }
}));
...
orderRouter.put('/:id/deliver', isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
    ...
    if (order) {
        ...
        const updatedOrder = await order.save();
        res.send({ message: 'Tilaus toimitettu', order: updatedOrder });
    } else {
        res.status(404).send({ message: 'Order Not Found'});
        }
}));
